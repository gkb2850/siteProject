<template>
  <div class="container menu8-4-container">
    <div class="filter-wrap">
      <span>查询信息：</span>
      <el-input v-model="queryParams.ThemeName" placeholder="请输入投诉主题" style="width:200px;"></el-input>
      <el-select v-model="queryParams.ComplaintState" placeholder="请选择投诉状态" style="width:200px;" clearable @clear="setValueNull(1)">
        <el-option
          v-for="item in complaintStateList"
          :key="item.id"
          :label="item.value"
          :value="item.id">
        </el-option>
    ...
    <el-table border row-key="id" ref="multipleTable" :data="tableData" style="width: 100%" @selection-change="handleSelectionChange" :header-cell-style="{background:'#f5f5f5',color:'#606266','text-align':'center'}" size="mini" :cell-style="{'text-align':'center'}">
       ...
       <el-table-column :show-overflow-tooltip="true" prop="goodsType" label="订单号"  >
        <template slot-scope="scope">
            <span><a type="primary" @click="orderInfo(scope.row.orderID)" style="color:#409eff">{{scope.row.order && scope.row.order.orderCode}}</a></span>
        </template>
      </el-table-column>
    ...
    <el-dialog title="投诉详情信息" :visible.sync="dialog1" width="60%" class="dialog1 detail-warp" :close-on-click-modal="false">
      <el-form ref="form" :model="form" label-width="100px">
        ...
        <el-form-item label="内容：">
          <el-table border row-key="id" ref="multipleTable" :data="tableDatalist" style="width: 100%" @selection-change="handleSelectionChange" :header-cell-style="{background:'#f5f5f5',color:'#606266','text-align':'center'}" size="mini" :cell-style="{'text-align':'center'}">
            <!--  <el-table-column :show-overflow-tooltip="true" type="selection" > </el-table-column> -->
