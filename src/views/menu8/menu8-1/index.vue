<template>
  <div class="container menu8-1-container">
    <div class="filter-wrap">
      ...
      <el-date-picker v-model="selectDatetime" type="daterange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" @change="dataSearch" value-format="yyyy-MM-dd HH:mm:ss" format="yyyy-MM-dd HH:mm:ss"> </el-date-picker>
...
export default {
  ...
  methods: {
    ...
    searchTime (index) {
      ...
      let arr = []
      if (index === 1) {
        arr[0] = `${date.getFullYear()}-${(date.getMonth() + 1) < 10 ? '0' + (date.getMonth() + 1) : (date.getMonth() + 1)}-${(date.getDate() - index) < 10 ? '0' + (date.getDate() - index) : (date.getDate() - index)} 00:00:00`
        arr[1] = `${date.getFullYear()}-${(date.getMonth() + 1) < 10 ? '0' + (date.getMonth() + 1) : (date.getMonth() + 1)}-${(date.getDate() - index) < 10 ? '0' + (date.getDate() - index) : (date.getDate() - index)} 00:00:00`
        this.selectDatetime = arr
      } else {
        arr[0] = `${date.getFullYear()}-${(date.getMonth() + 1) < 10 ? '0' + (date.getMonth() + 1) : (date.getMonth() + 1)}-${(date.getDate() - index) < 10 ? '0' + (date.getDate() - index) : (date.getDate() - index)} 00:00:00`
        arr[1] = `${date.getFullYear()}-${(date.getMonth() + 1) < 10 ? '0' + (date.getMonth() + 1) : (date.getMonth() + 1)}-${date.getDate() < 10 ? '0' + date.getDate() : date.getDate()} 00:00:00`
        this.selectDatetime = arr
      }
      this.query()
    }
