<template>
  <div class="container menu2-1-container">
    ...
    <el-table :data="dataList" style="width: 100%;margin-top:30px;" border size="mini" v-loading="loading">
    ...
    <el-dialog width="70%" :title="form.rechargeAttrID ? '修改接口' : '新增接口' " :visible.sync="dialogVisible">
      <el-form ref="form" :model="form" :rules="rules">
        <el-form-item label="充值标识" label-width="120px" prop="rechargeKey">
          <el-select v-model="form.rechargeKey" @change="changeKey" style="width:250px" size="small" placeholder="请选择充值标识" filterable>
            <el-option
        ...
        <el-form-item label="充值费率" label-width="120px" prop="rechargeRate">
                    <el-input-number style="width:250px" size="small" v-model="form.rechargeRate" controls-position="right" :min="0" :max="100"></el-input-number>
                </el-form-item>
                <el-form-item label="最低费用" label-width="120px" prop="rechargeCost">
                    <el-input-number style="width:250px" size="small" v-model="form.rechargeCost" controls-position="right" :precision="2" :step="0.01" :min="0"></el-input-number>
                </el-form-item>
        ...
        <el-form-item>
          <el-button size="small" style="float:right;margin-left:20px;" type="primary" @click="save('form')" :loading="loadingButton">{{ form.rechargeAttrID ? "立即保存" : "立即创建" }}</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
...
export default {
  ...
  methods: {
        ...
        getDatas({},'/finance/rechargeMode/list').then(res => {
            if (res.code==1001 && res.body){
              res.body.forEach(item => {
                item.rechargeField = item.rechargeField.split(",")
              })
                this.rechargeMode = res.body
            }
        }).catch(err => {
        })
    },
    // 切换标识
    changeKey () {
      let rateArr = this.handleData(this.attrJson,this.form)
