  ...
  <div class="container">
    ...
    <el-table row-key="id" ref="multipleTable" :data="tableData" style="width: 100%" @selection-change="handleSelectionChange" :header-cell-style="{background:'#eef1f6',color:'#606266','text-align':'center'}" v-loading="loading" size="mini" :cell-style="{'text-align':'center'}">
       ...
       <el-table-column :show-overflow-tooltip="true" prop="test8" label="操作" align="center">
        <template slot-scope="scope">
          ...
          <el-button type="text" size="mini" @click="del(scope.row.catalogRecommendID)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="goodsListDialogName" :visible.sync="goodsListDialog" width="60%" >
      <el-table row-key="goodsID" ref="multipleTable" :data="dataGoodsList" style="width: 100%" :header-cell-style="{background:'#eef1f6',color:'#606266','text-align':'center'}" v-loading="loadingGoods" size="mini" :cell-style="{'text-align':'center'}">
       ...
       <el-table-column :show-overflow-tooltip="true" prop="test8" label="操作" align="center">
        <template slot-scope="scope">
          <el-button type="text" size="mini" @click="del(scope.row.goodsID)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination background @size-change="handleSizeChangeGoods" @current-change="handleCurrentChangeGoods" :current-page="currentPage" :page-sizes="[10]" :page-size="goodsQuery.pageSize" layout="total, sizes, prev, pager, next, jumper" :total="goodsQuery.total"> </el-pagination>
    ...
    <el-dialog :title="goodsListDialogName" :visible.sync="addGoodsListDialog" width="60%" >
      <div class="filter-wrap">
        <span>商品名称：</span>
        <el-input v-model="goodsQuery.goodsName" placeholder="请输入商品名称" style="width:200px" class="search-input"></el-input>

        <span style="margin-left:10px">供货商名称：</span>
        <el-input v-model="goodsQuery.Supplier" placeholder="请输入供货名称" style="width:200px" class="search-input"></el-input>
        
        <span style="margin-left:10px">商品类型：</span>
        <el-select v-model="goodsQuery.goodsTypeID" placeholder="请选择商品类型" style="width:200px" clearable @clear="setValueNull(1)">
          <el-option
    ...
    <el-pagination background @size-change="handleSizeChangeGoodss" @current-change="handleCurrentChangeGoodss" :current-page="currentPage" :page-sizes="[10]" :page-size="goodsQuery.pageSize" layout="total, sizes, prev, pager, next, jumper" :total="goodsQuery.total"> </el-pagination>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    getGoodsList(item,index){
      ...
      getGoodsListCatalogue(this.goodsQuery).then(res => {
        if (res.code === 1001) {
          this.dataGoodsList = res.body.records
          this.goodsQuery = {
            ...this.goodsQuery,
