<template>
  <div class="container">
    ...
    <el-table row-key="catalogID" ref="multipleTable" :data="tableData" style="width: 100%" @selection-change="handleSelectionChange" :header-cell-style="{background:'#eef1f6',color:'#606266'}" v-loading="loading" :load="load" lazy :tree-props="{children: 'children', hasChildren: 'hasChildren'}">
       ...
       <el-table-column :show-overflow-tooltip="true" prop="catalogName" label="目录名称" width="300">
        <template slot-scope="scope">
            <span :style="{'color':scope.row.color}">{{scope.row.catalogName}}1111</span>
        </template>
      </el-table-column>
    ...
    <el-dialog title="修改目录" :visible.sync="colorDialog" :close-on-click-modal="false">
      <el-form ref="colorForm" :model="colorForm" label-width="160px">
          <el-form-item label="目录名称颜色：" prop="color" :rules="[{ required: true, message: '请目录名称的颜色', trigger: 'blur' }]">
              <el-color-picker v-model="colorForm.color"></el-color-picker>
          </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
    ...
    <el-dialog title="修改目录" :visible.sync="updateDialog" :close-on-click-modal="false">
      <el-form ref="updateForm" :model="updateForm" :rules="addCatalogFormRules" label-width="160px">
        <el-form-item label="商品目录名称：" prop="catalogName">
          ...
          <el-color-picker v-model="updateForm.color || color" @change="themeEvent"></el-color-picker>
        </el-form-item>
        <el-form-item label="上级目录选择：">
...
import { geTcatalogList, updateCatalog, deleteCatalog, updateStateBatch, updateAgentTypeBatch, AddCatalog, AddColorFormAPI, sortCatalog, sortCatalogArrow } from '@/api/meun3/menu3-1/index'
